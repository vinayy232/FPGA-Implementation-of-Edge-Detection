...
int main(){
	...
	XScuGic_SetPriorityTriggerType(&IntcInstance,XPAR_FABRIC_EDGEDETECTION_0_O_INTR_INTR,0xA0,3);
	status = XScuGic_Connect(&IntcInstance,XPAR_FABRIC_EDGEDETECTION_0_O_INTR_INTR,(Xil_InterruptHandler)imageProcISR,(void *)&myDma);
	if(status != XST_SUCCESS){
		xil_printf("Interrupt connection failed");
		return -1;
	}
	XScuGic_Enable(&IntcInstance,XPAR_FABRIC_EDGEDETECTION_0_O_INTR_INTR);
...
static void imageProcISR(void *CallBackRef){
	static int i=4;
	int status;
	XScuGic_Disable(&IntcInstance,XPAR_FABRIC_EDGEDETECTION_0_O_INTR_INTR);
